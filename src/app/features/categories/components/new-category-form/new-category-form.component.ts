...
export class NewCategoryFormComponent implements OnInit {
  ...
  add(){
    ...
    this.categoriesService.add(newCategory).subscribe({
      next: (addedCategory) => {
        //Next: Observable yapı herhangi bir değer döndürüğünde çalışır.
        console.log('Category added', addedCategory);
      },
      error: (error) => {
        //Error: Observable yapı ata verdiğinde çalışır.
        console.log('Error adding category', error);
      },
      complete: () => {
        //Complete: Observable yapı tamamen bittiğinde, bir daha next çalışmadığında,  çalışır.
